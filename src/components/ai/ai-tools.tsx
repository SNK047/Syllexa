    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          <Button variant={activeTab === "summary" ? "default" : "outline"} size="sm" onClick={() => loadTool("summary")}>
            <FileText className="h-3 w-3 mr-1" /> Summary
          </Button>
          <Button variant={activeTab === "flashcards" ? "default" : "outline"} size="sm" onClick={() => loadTool("flashcards")}>
            <Brain className="h-3 w-3 mr-1" /> Flashcards
          </Button>
          <Button variant={activeTab === "quiz" ? "default" : "outline"} size="sm" onClick={() => loadTool("quiz")}>
            <Lightbulb className="h-3 w-3 mr-1" /> Quiz
          </Button>
          <Button variant={activeTab === "keywords" ? "default" : "outline"} size="sm" onClick={() => loadTool("keywords")}>
            <Sparkles className="h-3 w-3 mr-1" /> Keywords
          </Button>
        </div>
...
function Flashcard({ card, index }: { card: any; index: number }) {
  ...
  const difficultyColor = card.difficulty === "easy" ? "text-green-500" : card.difficulty === "hard" ? "text-red-500" : "text-yellow-500";

  return (
    <div className="rounded-lg border border-border/50 p-4 cursor-pointer hover:border-primary/50 transition-colors" onClick={() => setFlipped(!flipped)}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="text-xs text-muted-foreground">#{index + 1}</span>
        <Badge variant="outline" className={`text-xs ${difficultyColor}`}>{card.difficulty}</Badge>
      </div>
      <p className="text-sm font-medium">{flipped ? card.answer : card.question}</p>
      <p className="text-xs text-muted-foreground mt-2">{flipped ? "Click to see question" : "Click to reveal answer"}</p>
    </div>
  );
}
    ...
    <div className="rounded-lg border border-border/50 p-4 space-y-3">
      ...
      <div className="space-y-2">
        {question.options.map((opt: string, i: number) => {
          ...
          if (showResult && isCorrect) className += "border-green-500 bg-green-500/10 text-green-600";
          else if (showResult && isSelected && !isCorrect) className += "border-red-500 bg-red-500/10 text-red-600";
          else className += "border-border/50 hover:border-primary/50";
          return (
            <button key={i} className={className} onClick={() => !showResult && setSelected(i)} disabled={showResult}>{opt}</button>
          );
        })}
      </div>
      {showResult && question.explanation && (
        <p className="text-xs text-muted-foreground bg-muted/50 rounded-lg p-2">{question.explanation}</p>
      )}
    </div>
  );
}
