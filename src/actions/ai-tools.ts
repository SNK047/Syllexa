...
async function getNoteContent(noteId: string): Promise<{ content: string | null; error?: string }> {
  ...
  return { content: null, error: "PDF text has not been extracted yet. Please re-upload this note to enable AI features." };
}
...
export async function createSummary(noteId: string, style: "brief" | "detailed" | "exam" = "brief") {
  const { content, error } = await getNoteContent(noteId);
